...
error_reporting(0);
            ...
            //get district name
            $g_id = $_GET['thanaid'];
            echo "<select class='box' id='offNsales' name='offNsales' style='width: 200px;font-family: SolaimanLipi !important;'>
           ...
           $sel_office_by_thana = mysql_query("SELECT * FROM office WHERE Thana_idThana = $g_id ORDER BY office_name ASC");
            while($office_rows = mysql_fetch_array($sel_office_by_thana))
                    {
                        $db_office_id = $office_rows['idSales_store'];
                        $db_office_name = $office_rows['salesStore_name'];
